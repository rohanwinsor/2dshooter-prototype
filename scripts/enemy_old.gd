...
var player_visible_time: float = 0.0  #
...
func change_state(new_state: State) -> void:
	print("Chansing State ::", new_state)
	if state == new_state:
		return
...
func enter_hunting_state(_from_state: State) -> void:
	navigate_to_postion(shot_position)
...
func is_shot_near(shot_position: Vector2) -> void:
	print("global_position.distance_to(shot_position) ::", global_position.distance_to(shot_position))
	if global_position.distance_to(shot_position) < 300:
